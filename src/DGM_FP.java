...
public class DGM_FP {
			...
			try {
				while (!goingToLeave) {
					...
					for (Integer key : heartbeatCnt.keySet()) {
						...
						if (curTime - heartbeatCnt.get(key) > 500) {
							heartbeatCnt.put(key, curTime);
							markAsFailure(key);
						}
					}
	...
	private static int lossRate;

	public static void main(String[] args) {
		lossRate = Integer.parseInt(args[0]);
		DGM_FP dgm = new DGM_FP();
		dgm.start();
	}
	...
	public void markAsFailure(Integer hashID) {
		if (membershipList[hashID] != null) {
			...
			writeLogs("Failure:", membershipList[hashID]);
		}
